...
export const TreeCheckbox = ({ size = 1.6, variant = "primary", bold = false, children, isDisabled, className, style, ...props }) => {
    ...
    const checkboxClassName = composeRenderProps(className, (resolved) => classNames('checkbox-label', { 'checkbox-label--disabled': isDisabled }, resolved));
    ...
    const checkboxStyle = composeRenderProps(style, (resolvedStyle) => ({
    ...
    return (_jsxs(RACCheckbox, { ...props, className: checkboxClassName, style: checkboxStyle, isDisabled: isDisabled, children: [_jsx("div", { "data-slot": "selection" }), children] }));
};
