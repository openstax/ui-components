...
const revisitStyleCss = '.cky-btn-revisit-wrapper.cky-btn-revisit-wrapper, .cky-btn-revisit { display: none; }';
...
const getRevisitStyle = () => document.head.querySelector(`#${revisitStyleId}`);
...
export const ManageCookiesLink = ({ children, className, wrapper, ...props }) => {
    const inBrowser = typeof window === 'object';
    const [cookieYesLoaded, setCookieYesLoaded] = React.useState(false);
    const buttonRef = React.useRef(null);
    const observerRef = React.useRef(null);
    const timeoutIdRef = React.useRef(null);
    const observerTimeoutIdRef = React.useRef(null);
    ...
    if (!cookieYesLoaded) {
        return _jsx("style", { dangerouslySetInnerHTML: { __html: revisitStyleCss } });
    }
    const button = _jsx(ButtonLink, { ref: buttonRef, className: `cky-banner-element${className ? ` ${className}` : ''}`, ...props, onClick: handleClick, children: children || 'Manage Cookies' });
