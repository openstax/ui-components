...
export const stripNoise = (css) => blankNoise(css, false);
/**
...
export const takesColor = (property) => {
 ...
 */
export const describeColor = (literal) => {
 ...
 */
export const findColors = (value, named) => {
    const found = [];
    let index = 0;
 ...
 */
export const stylesheetColors = (css) => {
    const found = [];
 ...
 * key.
 */
export const colorKey = (rgba) => rgba.a === 1 ? hex(rgba) : `${hex(rgba)}/${rgba.a}`;
/** Key ignoring alpha, so `rgba(0, 0, 0, 0.2)` can be recognised as the theme's black. */
export const opaqueKey = (rgba) => hex(rgba);
