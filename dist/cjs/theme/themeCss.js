...
exports.renderThemeCss = exports.themeTokens = exports.themeTokenGroups = void 0;
 ...
 */
exports.themeTokenGroups = [
    {
        label: "palette — src/theme/palette.ts",
        tokens: Object.entries(palette_1.palette).map(([key, value]) => [`--ox-color-${kebab(key)}`, value]),
    },
    {
        label: "link colours — src/theme.ts",
        tokens: [
            ["--ox-color-link", theme_1.colors.link.color],
            ["--ox-color-link-hover", theme_1.colors.link.hover],
        ],
    },
    {
        label: "z-index scale — src/theme.ts",
        tokens: Object.entries(theme_1.zIndex).map(([key, value]) => [`--ox-z-index-${kebab(key)}`, String(value)]),
    },
    {
        label: "padding — src/theme.ts",
        tokens: [
            ["--ox-padding-navbar-mobile", `${theme_1.padding.navbar.mobile}rem`],
            ["--ox-padding-navbar-desktop", `${theme_1.padding.navbar.desktop}rem`],
        ],
    },
];
/** Every token as a flat name -> value map. */
const themeTokens = () => new Map(exports.themeTokenGroups.flatMap((group) => group.tokens));
...
const renderThemeCss = () => {
    const body = exports.themeTokenGroups
        .map(({ label, tokens }) => [`  /* ${label} */`, ...tokens.map(([name, value]) => `  ${name}: ${value};`)].join("\n"))
        .join("\n\n");
    return `${header}\n:root {\n${body}\n}\n`;
};
