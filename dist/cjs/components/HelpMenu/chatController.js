...
exports.useChatController = void 0;
 ...
 */
const usePostMessageChannel = (popup, path) => {
    // This is used to validate messages and restrict postMessage target
    const popupOrigin = react_1.default.useMemo(() => (path ? new URL(path).origin : undefined), [path]);
    const sendMessage = react_1.default.useCallback((message) => {
        if (!popup.current || !popupOrigin)
            return;
 ...
 */
const useChatController = (path, preChatFields) => {
    const popup = react_1.default.useRef(null);
     ...
     */
    const sendPreChatFields = react_1.default.useCallback(() => {
     ...
     */
    const openChat = react_1.default.useCallback(() => {
        if (popup.current || !path)
            return;
