...
`;function V({onClick:t,className:o}){return c("div",{className:o,children:c("button",{type:"button",onClick:t,"aria-label":"close form",children:"Back"})})}const X=g(V)`
...
`,Z=({contactFormParams:t,chatConfig:o,children:e})=>{const[n,s]=a.useState(),{chatEmbedPath:r,businessHours:i,err:u}=a.useMemo(()=>o!=null?o:{},[o]),d=N(i),h=a.useMemo(()=>j(t),[t]),{openChat:m}=W(r,h),f=a.useMemo(()=>{const p="https://openstax.org/embedded/contact",I=t.map(({key:v,value:H})=>encodeURIComponent(`${v}=${H}`)).map(v=>`body=${v}`).join("&");return`${p}?${I}`},[t]);return a.useEffect(()=>{const p=({data:I})=>{I==="CONTACT_FORM_SUBMITTED"&&s(void 0)};return window.addEventListener("message",p,!1),()=>window.removeEventListener("message",p,!1)},[]),u&&console.error("Error getting chat config",u),w(E,{children:[w(z,{label:"Help","aria-label":"Help menu",children:[d&&m?w(T,{onAction:()=>m(),children:["Chat With Us (",d,")"]}):c(T,{onAction:()=>s(f),children:"Report an issue"}),e]}),n&&w(G,{children:[c(Y,{title:"Contact form",src:n}),c(X,{onClick:()=>s(void 0)})]})]})},q=M`
  [data-portal-slot="nav"] {
    position: fixed;
    top: 0;
    width: 100%;
  }
`,J={businessHoursInfo:{businessHours:[{startTime:Date.now()-6e4,endTime:Date.now()+144e4}]},timestamp:Date.now()},K=[{key:"userId",value:"test"},{key:"userFirstName",value:"test"},{key:"organizationName",value:"org"}],Q="https://localhost/assignable-chat",ee={chatEmbedPath:Q,businessHours:J},fe=()=>w(D.Provider,{value:["nav","root"],children:[c(q,{}),c($,{logo:!0,children:c(Z,{contactFormParams:K,chatConfig:ee,children:c(T,{onAction:()=>window.alert("Ran HelpMenu callback function"),children:"Test Callback"})})})]});typeof window<"u"&&window.document&&window.document.createElement&&document.documentElement.setAttribute("data-storyloaded","");export{fe as Default};
